...
const sidebarMenuButtonVariants = cva(
  'peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md text-left outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-ma-red/30 data-[active=true]:shadow-[inset_2px_-2px_16px_0px_rgba(255,_255,_255,_0.75)] data-[active=true]:font-medium data-[active=true]:text-ma-red data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:shrink-0 touch-hitbox active:scale-[0.97]',
  {
    variants: {
      variant: {
      ...
      size: {
        default: `
          h-12 px-4 text-base [&>svg]:size-5
          md:h-8 md:px-2 md:text-sm md:[&>svg]:size-4 
        `,
        sm: `
          h-10 px-3 text-sm [&>svg]:size-4
          md:h-7 md:px-2 md:text-xs md:[&>svg]:size-3
        `,
        lg: `
          h-14 px-5 text-lg [&>svg]:size-6
          md:h-12 md:px-4 md:text-sm md:[&>svg]:size-5
...
function SidebarMenuButton({
  ...
  const Comp = asChild ? Slot : 'button';
  const { isMobile, setOpen, state } = useSidebar(); // ✅ get setOpen

  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    props.onClick?.(e); // keep any existing click behavior
    if (isMobile) {
