...
import { EventRegistration, EventQuestion } from "@/lib/queries/registrations";
...
export const registrationsColumns = (questions: EventQuestion[]): ColumnDef<EventRegistration>[] => [
  // User Info Section
  {
    ...
    cell: ({ row }) => {
      ...
      const displayName = registration.userName || 
  ...
  ...questions.map((question): ColumnDef<EventRegistration> => ({
    ...
    cell: ({ row }) => {
      ...
      if (question.type === 'YesNo') {
        return (
          <Badge variant={response.toLowerCase() === 'yes' ? 'default' : 'secondary'}>
            {response}
          </Badge>
        );
      }
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem
              onClick={() => navigator.clipboard.writeText(registration.userEmail)}
            >
              Copy email
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => navigator.clipboard.writeText(registration.id.toString())}
            >
