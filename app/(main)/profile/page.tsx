      ...
      <CardContent className="p-12 flex flex-col gap-6 items-start">
        ...
        <div className="space-y-1">
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-sm">
            <span className="bg-muted px-2 py-1 rounded">Faculty: {user.faculty}</span>
            <span className="bg-muted px-2 py-1 rounded">Year: {user.yearLevel}</span>
            <span className="bg-muted px-2 py-1 rounded">Major: {user.major}</span>
          </div>

          {user.linkedinUrl && (
            <Link href={user.linkedinUrl} target="_blank" rel="noopener noreferrer">
              <Button variant="link" className="mt-2 bg-blue-200">
                LinkedIn Profile <ExternalLink className="w-4 h-4 ml-1" />
              </Button>
            </Link>
          )}
        </div>
